...
void init_oai_emulation(void)
{
  ...
  oai_emulation.environment_system_config.fading.free_space_model_parameters.pathloss_exponent = 3.00;//CHANGE ZYB FROM 3.00 to 2
  oai_emulation.environment_system_config.fading.free_space_model_parameters.pathloss_0_dB = -100;//change zyb from -100 to -50
  ...
  oai_emulation.environment_system_config.antenna.eNB_antenna.antenna_gain_dBi = 0;//CHANGE ZYB FROM 0 to 16
  oai_emulation.environment_system_config.antenna.eNB_antenna.tx_power_dBm = 15;//CHANGE ZYB FROM 15 to 40
  oai_emulation.environment_system_config.antenna.eNB_antenna.rx_noise_level_dB = 0;//CHANGE ZYB FROM 0 to 5
  ...
  oai_emulation.topology_config.area.x_m = 1000;//CHANGE ZYB FROM 1000 to 500 
  oai_emulation.topology_config.area.y_m = 1000;//CHANGE ZYB FROM 1000 to 500
  ...
  oai_emulation.info.nb_ue_local= 1;//default 1 UE       //cxh set ue numbers
  ...
  oai_emulation.info.ocm_enabled=0;// flag ? change zyb from 1
  ...
  for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
    oai_emulation.info.frame_type[CC_id]=FDD;
    oai_emulation.info.frame_type_name[CC_id]="FDD";
    oai_emulation.info.tdd_config[CC_id]=3;
    oai_emulation.info.tdd_config_S[CC_id]=0;
    oai_emulation.info.extended_prefix_flag[CC_id]=0;//扩展CP
#if bandwidth10
    oai_emulation.info.N_RB_DL[CC_id]=50;
#elif bandwidth20
    oai_emulation.info.N_RB_DL[CC_id]=100;
#else
    oai_emulation.info.N_RB_DL[CC_id]=25;
#endif
    oai_emulation.info.transmission_mode[CC_id]=1;//change zyb
  }
...
void oaisim_config(void)
{
 ...
 // oai_emulation.info.otg_enabled = 1; change zyb
    ...
    for (i=0; i<g_otg->num_nodes; i++) {
      for (j=0; j<g_otg->num_nodes; j++) {
        ...
        for (k=0; k<g_otg->application_idx[i][j]; k++) {
          ...
          //make change REPEAT_STRING
          g_otg->packet_gen_type=REPEAT_STRING;
