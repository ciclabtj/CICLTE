...
uint16_t           Nid_cell              = 0; //needed by init_lte_vars change zyb
int           nb_antennas_rx        = 1; // change zyb
...
void update_otg_eNB(module_id_t enb_module_idP, unsigned int ctime)
{
  ...
  if (oai_emulation.info.otg_enabled ==1 ) {
    ...
    for (dst_id = 0; dst_id < NUMBER_OF_UE_MAX; dst_id++) {
      ...
      //make change RRC connected
      int RRC_state;
      RRC_state = mac_eNB_get_rrc_status(enb_module_idP, oai_emulation.info.eNB_ue_module_id_to_rnti[enb_module_idP][dst_id]);
//      if (dst_id == 1)
//      {
//          RRC_state = 3;
//      }
