/*******************************************************************************
 * File  Name       :  3gpplte_turbo_decoder.c
 * Function Comments:  
 * Create Time      :  2012/7/1
 * Author           :  Wanlin,Tongji  University
 * Revision History :
 *    1.Created draft 2012/7/1
 * 
 * Function List    :
 *    1. function name: compute_gamma; 
 *    2. function name: compute_alpha;
 *    3. function name: compute_beta;
...
__m128i mtmp[MAX_DECODING_THREADS],mtmp2[MAX_DECODING_THREADS],lsw[MAX_DECODING_THREADS],msw[MAX_DECODING_THREADS],naw[MAX_DECODING_THREADS],mb[MAX_DECODING_THREADS],newcmp[MAX_DECODING_THREADS] __attribute__ ((aligned(16)));
  ...
  for (k=0;k<(frame_length+3);k+=8)
	...
	print_shorts("m10_max",&m10_max);
      */
      ...
      __m128i temp;
      temp = _mm_subs_epi16(m10_max[inst],_mm_adds_epi16(m01_max[inst],*systematic_128));
      memcpy(ext_128, &temp, sizeof(__m128i));
      /*
