                             ...
                             uint8_t llr8_flag)
{
  ...
  unsigned int r,r_offset=0,Kr,Kr_bytes, crc_len, crc, oldcrc;
  uint8_t crc_type;
  uint8_t *columnset;
...
#else
    tc = phy_threegpplte_turbo_decoder;
#endif
  }
  else
  ...
  */
  ...
  #pragma omp parallel private(r,ret) shared(ulsch,harq_pid,crc_type,Kr,f1f2mat_old,phy_vars_eNB,status,iind,)
  {
    ...
    for (r=0; r<ulsch_harq->C; r++) {
     ...
     static int index = 1;
     char fn[20];
    if(index%2 ==0 && index > 1000 && index < 1200){
        sprintf(fn, "data/data_%d", index);
        FILE * befof = fopen(fn, "a+");
         short *fd = &ulsch_harq->d[r][96];
        for(int i = 0; i < (Kr+4)*3; i++){
            fprintf(befof, "%d ", *fd++);
        }
        fclose(befof);
    }
...
#endif
      // printf("ret %d\n", ret);
      //printf("sf: %d byte needs %dus\n", Kr>>3, (e_time.tv_nsec-b_time.tv_nsec)/1000);

      FILE *fp = fopen("TIME/sdecoding.txt", "a+");
    ...
    task = get_task();
    task->ulsch = ulsch_harq;
    
    clock_gettime(CLOCK_REALTIME, &task->b_time);
    short *decoder_input = &ulsch_harq->d[r][96];
    unsigned char *pwp = (unsigned char *)task->addr;
#if 0
     static int index = 1;
     char fn[20];
    if(index%2 ==0 && index > 1000 && index < 1400){
        sprintf(fn, "data/move_%d", index/2);
        FILE * befof = fopen(fn, "a+");
         short *fd = &ulsch_harq->d[r][96];
        for(cpy = 0; cpy < (Kr+4)*3; cpy++){
            fprintf(befof, "%d\n", (*fd>>3));
            fd++;
        }
        fclose(befof);
...
#if 0
        remove("TIME/decode_input");
        FILE * befof = fopen("TIME/decode_input", "a+");
        pwp = (unsigned char *)task->addr;
        for(cpy = 0; cpy < (Kr+4)*4; cpy++){
            fprintf(befof, "%d\n", *pwp++);
        }
        fclose(befof);
    ...
    static int hacount = 0;
    static int rdcount = 0;
    F = (r==0) ? ulsch_harq->F : 0;
    ret = decoder_hardware(task, codelen);  
    printf("code length %d\n", Kr>>3);
#ifndef MULTIUSER
     //need change
    single_read(task);
    //printf("single_read %d\n", rdcount++);
    memcpy(ulsch_harq->c[r], task->addr, task->length);
    
    oldcrc= *((unsigned int *)(&ulsch_harq->c[r][(Kr>>3)-crc_len]));
    switch (crc_type) {
        case CRC24_A: 
          oldcrc&=0x00ffffff;
          crc = crc24a(&ulsch_harq->c[r][F>>3], Kr-24-F)>>8;
          temp=((uint8_t *)&crc)[2];
        ...
        case CRC24_B:
          oldcrc&=0x00ffffff;
          crc = crc24b(ulsch_harq->c[r], Kr-24)>>8;
          temp=((uint8_t *)&crc)[2];
          ((uint8_t *)&crc)[2] = ((uint8_t *)&crc)[0];
          ((uint8_t *)&crc)[0] = temp;
          break;
    ...
    if ((crc == oldcrc) && (crc!=0)) 
        ret = 2;
    else ret = ulsch->max_turbo_iterations + 1;
    
    clock_gettime(CLOCK_REALTIME, &task->e_time);
/*
    printf("hw: %d byte needs data trans-%dus, decoder-%dus, total-%dus\n", task->length, 
            (m_time.tv_nsec-task->b_time.tv_nsec)/1000,
            (task->e_time.tv_nsec-m_time.tv_nsec)/1000,
            (task->e_time.tv_nsec-task->b_time.tv_nsec)/1000);
*/
    FILE *fp = fopen("TIME/vhdecoding.txt", "a+");
    fprintf(fp, "%lld\n", task->e_time.tv_nsec-task->b_time.tv_nsec);
    fclose(fp);
    free_task(task);
#endif
#endif
                             ...
                             uint16_t *crnti)
{
 ...
 /*   1    unsigned char flag;
             if(*decoder_input != 0){
                ...
                printf("flag: %02x\n", flag);
                printf("%04x\n", *decoder_input);
                abs_input = abs(*decoder_input);
                printf("%04x\n", abs_input);
                position = get_position(abs_input);
                printf("position: %d\n", position);
                if(position < 8){
                    result = abs_input << 8;
                }else if(position == 8){
    ...
    2  *decoder_input = *decoder_input>>3;
        if(*decoder_input < -128) *pwp = 0x80;
        else if(*decoder_input > 127) *pwp = 0x7f;
        else *pwp = *decoder_input;
        pwp++;
        decoder_input++; 
    
     3     unsigned char flag;
           unsigned char *uptr = decoder_input;
          uptr++;
          flag = (0x80)&(*uptr);
            *decoder_input = *decoder_input<<5;
            *pwp++ = (*uptr>>1)|(flag);
            decoder_input++;
            uptr+=2;
     
     4  *pwp++ = *decoder_input>>6; decoder_input++;   //*decoder_input>>2; decoder_input++;
        pwp ++;
        *pwp = *decoder_input;
        if(*decoder_input & 0x8000) *pwp = 0x80;   //negative      
        else if(*decoder_input & 0x0080) *pwp = 0x7f;  //positive overflow
        pwp++;
        decoder_input++;
     */
