...
int send_c2 = 1, set_pack = 1;

double snr_estimation(short *test5, int size) {
    int index = 0;
    int sum_i = 0;
    int sum_q = 0;
    int avg_i = 0;
    ...
    if (index != 0) {
        ...
        for (int i = 0; i < index; i++) {
            noise_pow += (double) ((pow(data_i[i] - avg_i, 2) + pow(data_q[i] - avg_q, 2)) / index);
            signal_pow += (double) ((pow(data_i[i], 2) + pow(data_q[i], 2)) / index);
        }
        if (noise_pow > 0) {
            snr = (double) (10 * log10(signal_pow / noise_pow));
            //printf("snr:%f\n",snr);
            //FILE *fp = fopen("TIME/snr_estimation", "a+");
            //fprintf(fp, "%f\n", snr);
            //fclose(fp);
        }
    }
              ...
              uint8_t cooperation_flag)
{
  ...
  //cxh_bgn
  int cxh_nb_rb = ulsch[UE_id]->harq_processes[harq_pid]->nb_rb;
  //cxh_end
  for (l=0; l<(frame_parms->symbols_per_tti-ulsch[UE_id]->harq_processes[harq_pid]->srs_active); l++) {
    ...
    lte_ul_channel_estimation(phy_vars_eNB,
     ...
     //CXH_bgn   发送信道估计
     short *data_in;
    if (conn_flg && sched_subframe == 9) {
        int send_num;
        char d[sizeof (PHY_Data_Send)];
        if (qt_ue_id == 2){
            data_in = (short*) &(phy_vars_eNB->lte_eNB_pusch_vars[qt_ue_id - 1]->drs_ch_estimates[eNB_id][0][0]);
        }else {
            qt_ue_id = 0;
            data_in = (short*) &(phy_vars_eNB->lte_eNB_pusch_vars[qt_ue_id]->drs_ch_estimates[eNB_id][0][0]);
        }
        short loop;
        double psum = 0;
        ...
        //  FILE *fp;
        //    double psum = 0;
        // fp = fopen("chenck_chan_est.txt", "w+");
        for (int symb = 0; symb < 14; symb++) {
            for (loop = 0; loop < 300; loop++) {
                FFTData.chan_info[symb][loop][0] = data_in[ symb * 300 + (loop << 1)];
                FFTData.chan_info[symb][loop][1] = data_in[symb * 300 + (loop << 1) + 1 ];


                //                     if (loop == 0) {
                //                        fprintf(fp, "\r\n%d:     %d+%di  ", loop << 1, data_in[loop << 1], data_in[(loop << 1) + 1]);
                //                        continue;
                //                    }
                //                    if ((loop % 10) == 0) {
                //                        fprintf(fp, "\r\n%d:     %d+%di  ", loop << 1, data_in[loop << 1], data_in[(loop << 1) + 1]);
                //                    } else {
                //                        fprintf(fp, "%d+%di  ", data_in[loop << 1], data_in[(loop << 1) + 1]);
                //                    }
...
////  
  //CXH_bgn////////////////////////////////////////////////////////
    ...
    if (conn_flg && sched_subframe == 9) {
        short *test3;
        if (qt_ue_id == 2) {
            test3 = (short *) &phy_vars_eNB->lte_eNB_pusch_vars[qt_ue_id - 1]->rxdataF_comp[eNB_id][0][0];
        } else {
            //  非user2就认为是user1， 防止QT端传来了莫名其妙的ue id 导致这边内存错误    
            qt_ue_id = 0;
            test3 = (short *) &phy_vars_eNB->lte_eNB_pusch_vars[qt_ue_id ]->rxdataF_comp[eNB_id][0][0];
        }
        //            double  pdata[300][2];
        //            for(int i=0; i < 300 ;i++)
        //            {
        //                    pdata[i][0]=test3[i << 1];
        //                    pdata[i] [1] = test3[(i << 1) + 1];
        //            }
        int send_num;
        //           // msg("\nsymbol %d\n",symbol);
        //          //  if(symbol == 13)
        //            //        FFTData.send_c2 = send_c2++;
        //            
        //            
        //            double psum = 0,  avgp = 0;
        //            for(int i=0; i < 300 ;i++)
        //            {
        //                    psum+= exp(pdata[i][0]) + exp(pdata[i][1]);
        //            }
        //            avgp = sqrt(psum / 300);
        //            if(avgp != 0)
        //            {
        //                    for(int i=0; i < 300 ;i++)
        //                    {
        //                            pdata[i][0] = (pdata[i][0])/avgp;
        //                            pdata[i][1] = (pdata[i][1])/avgp;
        //                    }
        //            }
        //            else
        //                    printf("average power is zero , error \n");
        //            // 功率归一化完成
        //
        //        
        for (int sy = 0; sy < 14; sy++) {
            for (int i = 0; i < 300; i++) {   //300
                FFTData.cons_dia[sy][i][0] = test3[300 * sy + (i << 1)];
                FFTData.cons_dia[sy][i][1] = test3[300 * sy + (i << 1) + 1];
                //            FFTData.cons_dia[set_pack-1][i][0] = test3[i << 1];
                //            FFTData.cons_dia[set_pack-1][i][1] = test3[(i << 1) + 1];
            }
            double snr_data = snr_estimation((short *) &test3[300 * sy], cxh_nb_rb * 12); //edit by dingzhuang
            FFTData.snr_data[sy ] = snr_data;

        }


        //             FILE *cxhf;
        //                    cxhf = fopen("/home/cic/桌面/cxh_work/test/xingzuo", "a+");
        //                    for (int s = 0; s < 14; s++) {
        //                        for (int i = 0; i < 300; i++) {
        //                            fprintf(cxhf, " %d   %d\n", FFTData.cons_dia[s][i][0], FFTData.cons_dia[s][i][1]);
        //                        }
        //                        fprintf(cxhf, "snr:  %f\n", FFTData.snr_data[s]);
        //                    }
        //                    fclose(cxhf);
