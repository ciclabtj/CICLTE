...
void get_beta_map()
{
  ...
  for (mcs = 0; mcs < MCS_COUNT; mcs++) {
    //  make change add environment variable
      snprintf( file_path, 512, "/home/zhaiyubo/openairLTE_openair2_singlethread/openair1/SIMULATION/LTE_PHY/BLER_SIMULATIONS/AWGN/AWGN_results/bler_tx1_chan18_nrx1_mcs%d.csv", mcs );
...
void get_MIESM_param()
{
  ...
  for (q=0; q<3; q++) {
    //  make change add environment variable
    snprintf( file_path, 512, "/home/yichenqian/code/openairLTE_openair2_singlethread/openair1/SIMU/USER/files/MI_%dqam.csv", qam[q]);
