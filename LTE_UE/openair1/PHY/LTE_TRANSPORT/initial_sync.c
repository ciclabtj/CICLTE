...
int initial_sync_simu(PHY_VARS_UE *phy_vars_ue, runmode_t mode)
{
  ...
  if (((sync_pos2 - sync_pos_slot) >=0 ) &&
    ...
    ret = pbch_detection(phy_vars_ue,mode);//frame 6 同步 frame_mod4 = 2, crc=0
...
int initial_sync_1(PHY_VARS_UE *phy_vars_ue, runmode_t mode, int* syncOffset)
{
  ...
  LOG_I(PHY,"[UE%d] Initial sync : Estimated PSS position %d, sync_pos2: %d,Nid2 %d\n",phy_vars_ue->Mod_id,sync_pos,sync_pos2,phy_vars_ue->lte_ue_common_vars.eNb_id);
  ...
  if (sync_pos2 >= sync_pos_slot){
      ...
      *syncOffset = sync_pos2-sync_pos_slot;
      return -1;
  }
   // phy_vars_ue->rx_offset = FRAME_LENGTH_COMPLEX_SAMPLES + sync_pos2 - sync_pos_slot;
  ...
  if (((sync_pos2 - sync_pos_slot) >=0 ) &&
    ...
    ret = pbch_detection(phy_vars_ue,mode);
    // write_output("rxdata2.m","rxd2",phy_vars_ue->lte_ue_common_vars.rxdata[0],10*frame_parms->samples_per_tti,1,1);
    *syncOffset = phy_vars_ue->rx_offset;
#ifdef DEBUG_INITIAL_SYNCH
    LOG_I(PHY,"FDD Normal prefix: CellId %d metric %d, phase %d, flip %d, pbch %d\n",
          frame_parms->Nid_cell,metric_fdd_ncp,phase_fdd_ncp,flip_fdd_ncp,ret);
#endif
    ...
    LOG_I(PHY,"FDD Normal prefix: SSS error condition: sync_pos %d, sync_pos_slot %d\n", sync_pos, sync_pos_slot);
#endif
    *syncOffset = FRAME_LENGTH_COMPLEX_SAMPLES;
    return -1;
  }
...
int initial_sync(PHY_VARS_UE *phy_vars_ue, runmode_t mode, int* read_offset)
{
  ...
  FILE *fsync_pos = fopen( "TIME/fsync_pos","a+");
  fprintf(fsync_pos, "%d\n",sync_pos);
  fclose(fsync_pos);
 ...
 if (sync_pos >= frame_parms->nb_prefix_samples)
  ...
  if (sync_pos2 >= sync_pos_slot){
      ...
      *read_offset = sync_pos2-sync_pos_slot;
      return -1;
  }
   // phy_vars_ue->rx_offset = FRAME_LENGTH_COMPLEX_SAMPLES + sync_pos2 - sync_pos_slot;
  ...
  if (((sync_pos2 - sync_pos_slot) >=0 ) &&
    ...
    ret = pbch_detection(phy_vars_ue,mode);
    // write_output("rxdata2.m","rxd2",phy_vars_ue->lte_ue_common_vars.rxdata[0],10*frame_parms->samples_per_tti,1,1);
    *read_offset = phy_vars_ue->rx_offset;
    ...
    LOG_I(PHY,"FDD Normal prefix: SSS error condition: sync_pos %d, sync_pos_slot %d\n", sync_pos, sync_pos_slot);
#endif
    //*read_offset = FRAME_LENGTH_COMPLEX_SAMPLES;//  76800-offset
    *read_offset = FRAME_LENGTH_COMPLEX_SAMPLES - phy_vars_ue->rx_offset ;//  76800-offset
    return -1;
  }
