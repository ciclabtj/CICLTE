...
void fill_dci(DCI_PDU *DCI_pdu, uint8_t sched_subframe, PHY_VARS_eNB *phy_vars_eNB) {
    ...
    switch (subframe) {
        default:
            ...
            //
            //            ((DCI1_10MHz_FDD_t *) & dlsch_pdu)->rballoc = rballoc2;  /// RB Assignment (ceil(log2(N_RB_DL/P)) bits uint32_t rballoc:17;
            //            ((DCI1_10MHz_FDD_t *) & dlsch_pdu)->TPC = 1;  /// Power Control uint32_t TPC:2;
            //            ((DCI1_10MHz_FDD_t *) & dlsch_pdu)->harq_pid = 0;  /// HARQ Process uint32_t harq_pid:3;
            //            ((DCI1_10MHz_FDD_t *) & dlsch_pdu)->mcs = 4;  /// Modulation and Coding Scheme and Redundancy Version uint32_t mcs:5;
            //            ((DCI1_10MHz_FDD_t *) & dlsch_pdu)->ndi = 1;  /// New Data Indicator uint32_t ndi:1;
            //            ((DCI1_10MHz_FDD_t *) & dlsch_pdu)->rv = 1;  /// Redundancy version uint32_t rv:2;
            //            ((DCI1_10MHz_FDD_t *) & dlsch_pdu)->rah = 0;  /// Resource Allocation Header uint32_t rah:1;
    ...
    switch (subframe) {
        case 0:case 5:
            ...
            ((DCI1_5MHz_FDD_t *) & dlsch_pdu)->rballoc = 0x1edf;
            ...
            memcpy((void*) &DCI_pdu->dci_alloc[1].dci_pdu[0], (void *) &dlsch_pdu, sizeof (DCI1_5MHz_FDD_t));
            break;
...
void phy_procedures_eNB_TX(unsigned char sched_subframe, PHY_VARS_eNB *phy_vars_eNB, uint8_t abstraction_flag,
        ...
        if (DCI_pdu->dci_alloc[i].rnti == SI_RNTI) {
            ...
            for (UE_id = 0; UE_id < NUMBER_OF_UE_MAX; UE_id++) {
                if (UE_id >= 0) {
                    ...
                    printf("****[eNB %u][PDSCH %u /%u ] Frame %d subframe %d: Generated dlsch params\n",
                            phy_vars_eNB->Mod_id, DCI_pdu->dci_alloc[i].rnti, phy_vars_eNB->dlsch_eNB[(uint8_t) UE_id][0]->current_harq_pid, frame, subframe);
                    ...
                    if (result == -1) {
                        ...
                        printf("**Waring [eNB %d] Frame %d subframe %d : No available CCE resources for UE spec DCI (PDSCH %d) !!!\n",
                                phy_vars_eNB->Mod_id, frame, subframe, DCI_pdu->dci_alloc[i].rnti);
                    } else {
                        printf("[eNB %u] Frame %d subframe %d : CCE resource for ue DCI (PDSCH %d)  => %d/%u\n", phy_vars_eNB->Mod_id, frame, subframe,
    ...
    char name[100];
    if (abstraction_flag == 0) {
        //           sprintf(name,"txdataF/enb_sf%d",subframe);
        //           write_output(name,"w",&phy_vars_eNB->lte_eNB_common_vars.txdataF[0][0][subframe * 14 * 512],7168,1,1);
        ...
        if ((phy_vars_eNB->ulsch_eNB[i]) &&
            ...
            if (ret <= 4) {
                ...
                for (int ii = 0; ii < 15; ii++) {
                    printf("%c", phy_vars_eNB->ulsch_eNB[0]->harq_processes[harq_pid]->b[ii]);
                }
                printf("\n\n");
#endif          
            } else {
                printf(" uplink is  error\n");
            }
