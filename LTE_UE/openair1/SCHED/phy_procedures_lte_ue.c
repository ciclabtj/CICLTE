...
extern short prach_time_tx[6936 * 2];
extern short pprach[3][6936 * 2];

extern int oai_exit;
extern int ue_SI_flag; //make change zyb
...
void phy_procedures_UE_TX(PHY_VARS_UE *phy_vars_ue, uint8_t eNB_id, uint8_t abstraction_flag, runmode_t mode, relaying_type_t r_type) {
    ...
    if ((slot_tx % 2) == 0) {
        ...
        if (phy_vars_ue->UE_mode[eNB_id] != PRACH) {
            ...
            if (phy_vars_ue->ulsch_ue[eNB_id]->harq_processes[harq_pid]->subframe_scheduling_flag == 1) {
                ...
                if (Msg3_flag == 1) {
                    ...
                    LOG_E(PHY, "uplink:input_buffer_length:%d\n", input_buffer_length);
                    for (i = 0; i < input_buffer_length; i++)
                        ulsch_input_buffer[i] = 'C'; 
        ...
        if (phy_vars_ue->UE_mode[eNB_id] == PRACH) {//edit by ding
            //if(subframe_tx==1){
            //if (mode == PRACH) {
            ...
            if (is_prach_subframe(&phy_vars_ue->lte_frame_parms, frame_tx, subframe_tx)) {
                ...
                if (phy_vars_ue->prach_resources[eNB_id] != NULL) {
                    ...
                    if (abstraction_flag == 0) {
                        ...
                        // memcpy((short *) &phy_vars_ue->lte_ue_common_vars.txdata[0][7680], (short *) &prach_time_tx[0], 6936 * 2 * sizeof (short));//edit by ding                    
                        static unsigned short tt = 0;
                        memcpy((short *) &phy_vars_ue->lte_ue_common_vars.txdata[0][7680], (short *) &pprach[0 % NUMBER_OF_UE_MAX][0], 6936 * 2 * sizeof (short)); //edit by ding                    


                        //                    FILE *ppp;
                        //                    ppp = fopen("prach_test/prach_timesb.txt","w");                    
                        //                    for(int kp=0;kp<76800;kp++)
                        //                    {                                    
                        //                        fprintf(ppp,"%d,%d,\n",((short *)&phy_vars_ue->lte_ue_common_vars.txdata[0][7680])[2*kp],((short *)&phy_vars_ue->lte_ue_common_vars.txdata[0][7680])[2*kp+1]);                                
                        //                    }
                        //                    fclose(ppp);
...
void lte_ue_pbch_procedures(uint8_t eNB_id, PHY_VARS_UE *phy_vars_ue, uint8_t abstraction_flag) {
    ...
    if ((pbch_tx_ant > 0) && (pbch_tx_ant <= 4)) {
        ...
        if (phy_vars_ue->lte_ue_pbch_vars[eNB_id]->pdu_errors_conseq >= 1000) {
    ...
    LOG_E(PHY, "[UE %d] frame %d, slot %d, PBCH errors = %d, consecutive errors = %d!\n",
            phy_vars_ue->Mod_id, frame_rx, slot_rx,
            phy_vars_ue->lte_ue_pbch_vars[eNB_id]->pdu_errors,
            phy_vars_ue->lte_ue_pbch_vars[eNB_id]->pdu_errors_conseq);
#ifdef DEBUG_PHY_PROC
    LOG_D(PHY, "**************dingding***********************[UE %d] frame %d, slot %d, PBCH errors = %d, consecutive errors = %d!**************dingding***********************\n",
            phy_vars_ue->Mod_id, frame_rx, slot_rx,
            phy_vars_ue->lte_ue_pbch_vars[eNB_id]->pdu_errors,
            phy_vars_ue->lte_ue_pbch_vars[eNB_id]->pdu_errors_conseq);
#endif
...
int lte_ue_pdcch_procedures(uint8_t eNB_id, PHY_VARS_UE *phy_vars_ue, uint8_t abstraction_flag) {
    ...
    if (frame_rx == 1 && subframe_rx == 1)
        printf("aa\n");
    if (subframe_rx == 0 || subframe_rx == 9)
        printf("aa\n");
...
int phy_procedures_UE_RX(PHY_VARS_UE *phy_vars_ue, uint8_t eNB_id, uint8_t abstraction_flag, runmode_t mode,
    ...
    int slot_rx = phy_vars_ue->slot_rx;
    int subframe_rx = slot_rx >> 1;
    int subframe_prev = subframe_rx; //(subframe_rx+9)%10;
    ...
    for (l = 0; l < n_symb; l++) {
        ...
        if ((((slot_rx % 2) == 0) && ((l == pilot1))) ||
            ...
            if (lte_ue_pdcch_procedures(eNB_id, phy_vars_ue, abstraction_flag) == -1) {
                printf("****[UE  %d] Frame %d, slot %d: Error in pdcch procedures\n", phy_vars_ue->Mod_id, frame_rx, slot_rx);
                return (-1);
            }
        }
#if 1
        ...
        if (((slot_rx % 2) == 1) && (l == 6)) {
            ...
            if (phy_vars_ue->dlsch_ue[eNB_id][0]->active == 1) {
                ...
                if (phy_vars_ue->dlsch_ue[eNB_id][0]) {
                    ...
                    printf("****[UE] dlsch->active in subframe %d, ret: %d\n", subframe_prev, ret);
                    static double err_cnt = 0, total_cnt = 0;
                    total_cnt++;
                    static int record_next = 0;
                    if (ret == (1 + phy_vars_ue->dlsch_ue[eNB_id][0]->max_turbo_iterations)) {
                        FILE *ferr_sub = fopen( "TIME/ferr_record", "a+");
                        fprintf(ferr_sub, "%d\n", subframe_prev);
                        fclose( ferr_sub);
                        err_cnt++;
                        record_next = 1;
                        ...
                        phy_vars_ue->total_received_bits[eNB_id] = phy_vars_ue->total_TBS[eNB_id] +
                                phy_vars_ue->dlsch_ue[eNB_id][0]->harq_processes[phy_vars_ue->dlsch_ue[eNB_id][0]->current_harq_pid]->TBS;
                        printf("PDSCH\n");
                        //phy_vars_ue->dlsch_ue[eNB_id][0]->harq_processes[harq_pid]->TBS/8
                        for (int jj = 0; jj < 10; jj++)
                            printf("%c", phy_vars_ue->dlsch_ue[eNB_id][0]->harq_processes[harq_pid]->b[jj]);
                        
                        if( record_next == 1){
                            record_next = 0;
                            FILE *ferr_sub = fopen( "TIME/ferr_record", "a+");
                        for (int jj = 0; jj < 10; jj++)
                            fprintf(ferr_sub, "%c", phy_vars_ue->dlsch_ue[eNB_id][0]->harq_processes[harq_pid]->b[jj]);
                        fclose( ferr_sub);
                        }
                        //printf("\n");
                        if (subframe_rx ==4 ) {
                            //           if ((( int*) &phy_vars_ue->dlsch_ue[eNB_id][0]->harq_processes[harq_pid]->b[0])[0] != 87654  ) {  // cxh  enb填充包解析出来的TA值是87654
                            if (((int*) &phy_vars_ue->dlsch_ue[eNB_id][0]->harq_processes[harq_pid]->b[0])[0] < 76800 && ((int*) &phy_vars_ue->dlsch_ue[eNB_id][0]->harq_processes[harq_pid]->b[0])[0] > -76800) {
                                phy_vars_ue->cxh_TA = ((int*) &phy_vars_ue->dlsch_ue[eNB_id][0]->harq_processes[harq_pid]->b[0])[0];
                            } else {
                                phy_vars_ue->cxh_TA = 0;
                            }
                            //     printf("TA::::::%d\n", phy_vars_ue->cxh_TA);

                            if (phy_vars_ue->cxh_TA != 0) {
                                FILE *fp1 = fopen("TIME/TA1", "a+");
                                fprintf(fp1, "%d\n", phy_vars_ue->cxh_TA);
                                fclose(fp1);
                            }
                        }
                    }
                    printf("                                           err_cnt: %f, total_cnt: %f, err_rate: %f\n", err_cnt, total_cnt, err_cnt/total_cnt);
            ...
            // SI_DLSCH
            //phy_vars_ue->dlsch_ue_SI[eNB_id]->active = 1;
                ...
                if (phy_vars_ue->dlsch_ue_SI[eNB_id]) {
                    if (abstraction_flag == 0) {
                        ...
                        dlsch_unscrambling(&phy_vars_ue->lte_frame_parms,
                                ...
                                0,
                                subframe_prev << 1); //subframe_prev<<1); blake
                    ...
                    printf("****SI is active in subframe %d,ret: %d\n", subframe_prev, ret);
                    if (ret == (1 + phy_vars_ue->dlsch_ue_SI[eNB_id]->max_turbo_iterations)) {
                        phy_vars_ue->dlsch_SI_errors[eNB_id]++;
                        return (-1);
                    } else {
                        printf("SI:");
                        for (int jj = 0; jj < phy_vars_ue->dlsch_ue_SI[eNB_id]->harq_processes[0]->TBS >> 3; jj++)
                            printf("%c", phy_vars_ue->dlsch_ue_SI[eNB_id]->harq_processes[0]->b[jj]);
                        printf("\n");
                    }
                }
            }
            ...
            if (phy_vars_ue->dlsch_ue_ra[eNB_id]->active == 1) {
                printf("***[UE  %d] Frame %d, slot %d: DLSCH (RA) demod symbols 10,11,12\n", phy_vars_ue->Mod_id, frame_rx, slot_rx);
                // process symbols 10,11,12 and trigger DLSCH decoding
                if (abstraction_flag == 0) {
                    for (m = pilot3; m < phy_vars_ue->lte_frame_parms.symbols_per_tti; m++)
                        rx_pdsch(phy_vars_ue,
                ...
                if (ret == (1 + phy_vars_ue->dlsch_ue_ra[eNB_id]->max_turbo_iterations)) {
                    printf("****[UE  %d] Frame %d, subframe %d, received RA in error\n", phy_vars_ue->Mod_id, frame_rx, subframe_prev);
                    return (-1);
                } else {
            ...
            if (pmch_mcs >= 0) {
                ...
                if (abstraction_flag == 0) {
                    ...
                    dlsch_unscrambling(&phy_vars_ue->lte_frame_parms, 1, phy_vars_ue->dlsch_ue_MCH[0],
                            phy_vars_ue->dlsch_ue_MCH[0]->harq_processes[0]->G,
                            phy_vars_ue->lte_ue_pdsch_vars_MCH[0]->llr[0], 0, slot_rx);
...
#undef DEBUG_PHY_PROC
    ...
    prach_resources_local.ra_PreambleIndex = 1;
#endif
